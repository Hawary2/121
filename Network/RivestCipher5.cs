...
namespace AccServer.Network
{
    public unsafe static class NativeFunctionCalls
    {
        ...
        [DllImport("msvcrt.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern unsafe void* memset(void* dst, byte fill, int length);

        [DllImport("msvcrt.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern unsafe int memcmp(void* buf1, void* buf2, int count);
        ...
        [DllImport("msvcrt.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern unsafe void* memcpy(byte[] dst, byte[] src, int length);

        [DllImport("msvcrt.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern unsafe void* memcpy(uint[] dst, uint[] src, int length);

        [DllImport("msvcrt.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern unsafe void* memcpy(byte[] dst, string src, int length);
    ...
    public unsafe static class SeedGenerator
    {
        public static byte[] Generate(int seed)
        {
            ...
            for (int index = 0; index < 16; ++index)
            {
                seed *= 214013;
                seed += 2531011;
                numArray[index] = (byte)(seed >> 16 & (int)short.MaxValue);
            }
            return numArray;
        }
    }
