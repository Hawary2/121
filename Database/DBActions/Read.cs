...
namespace GameServer.Database.DBActions
{
    public class Read : IDisposable
    {
        ...
        public bool Reader(bool useinvalid = true)
        {
            ...
            if (Exit == File.Exists(location))
            {
                try
                {
                     using (SR = File.OpenText(location))
                    {
                        ...
                        {
                            MyConsole.WriteLine("[Error] An error occurred while reading " + location, ConsoleColor.Magenta);
                            SR.Close(); 
                            return false;
                        }
                        SR.Close();
            ...
            else if (useinvalid)
            {
                ...
                MyConsole.WriteLine("Invalid Reader " + location + " location", ConsoleColor.Magenta);
            }
